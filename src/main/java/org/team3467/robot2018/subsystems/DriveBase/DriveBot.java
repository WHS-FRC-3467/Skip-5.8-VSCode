...
public class DriveBot extends Command {
	...
	protected void initialize() {
		...
		Robot.driveBase.setDriveControlMode(m_driveMode);
		SmartDashboard.putString("Drive Mode", driveModeNames[m_driveMode]);
		SmartDashboard.putString("Talon Mode", Robot.driveBase.getTalonControlMode());
	}
	...
	protected void execute() {
		...
		default:
		case driveMode_Tank:
			Robot.driveBase.driveTank(getControllerLeftStickY(), getControllerRightStickY());
			break;
		...
		case driveMode_RocketSpin:
			...
			Robot.driveBase.drive(speed, getControllerLeftStickX(), (m_driveMode == driveMode_RocketSpin));
	...
	private double getControllerLeftStickX() {
		...
		return (m_lastCtrlLeftX = adjustStick(Robot.oi.getControllerLeftX(), m_lastCtrlLeftX, 0.20)); 

	}

	private double getControllerLeftStickY() {

		return (m_lastCtrlLeftY = adjustStick(Robot.oi.getControllerLeftY() * (-1.0), m_lastCtrlLeftY, 0.20)); 

	}

	private double getControllerRightStickX() {

		return (m_lastCtrlRightX = adjustStick(Robot.oi.getControllerRightX(), m_lastCtrlRightX, 0.20)); 

	}
	
	private double getControllerRightStickY() {

		return (m_lastCtrlRightY = adjustStick(Robot.oi.getControllerRightY() * (-1.0), m_lastCtrlRightY, 0.20)); 

	}
	
	private double getControllerLeftTrigger() {

		return (m_lastCtrlLeftTrig = adjustStick(Robot.oi.getControllerLeftTrigger(), m_lastCtrlLeftTrig, 0.20)); 

	}

	private double getControllerRightTrigger() {

		return (m_lastCtrlRightTrig = adjustStick(Robot.oi.getControllerRightTrigger(), m_lastCtrlRightTrig, 0.20)); 
